...
// ESM ignores NODE_PATH, so run it from a folder that has a node_modules with playwright, e.g.
//   cd <scratch> && ln -s ~/.npm/_npx/<hash>/node_modules node_modules && cp sliding-puzzle/record.mjs . \
//   && node record.mjs <name> [slideMs=85] [maxSeconds=90]
...
const [,, name = "take", slide = "85", maxSecs = "90"] = process.argv;
...
await p.goto(`http://127.0.0.1:3459/?demo=1&slide=${slide}`);
